...
app.config.update({
 #    'SECRET_KEY1': 'u\x91\xcf\xfa\x0c\xb9\x95\xe3t\xba2K\x7f\xfd\xca\xa3\x9f\x90\x88\xb8\xee\xa4\xd6\xe4',
     'SECRET_KEY': 'PZw37MlpVcjACg4MBJi1ecEcJlLYZlGT',
    'TESTING': True,
    'DEBUG': True,
    'OIDC_CLIENT_SECRETS': 'auth.json',
    'OIDC_ID_TOKEN_COOKIE_SECURE': False,
    'OIDC_REQUIRE_VERIFIED_EMAIL': False,
    'OIDC_USER_INFO_ENABLED': True,
    'OIDC_VALID_ISSUERS': ['http://localhost:8080/realms/flask-app'],
...
keycloak_openid = KeycloakOpenID(server_url="http://localhost:8080/",
                                 ...
                                 client_secret_key="PZw37MlpVcjACg4MBJi1ecEcJlLYZlGT")
...
def get_token():
    ...
    realm="flask-app"
    url="http://localhost:8080/realms/{0}/protocol/openid-connect/token".format(realm)
    ...
    url = "http://localhost:8080/realms/{0}/protocol/openid-connect/userinfo".format(realm)
    headers = {
...
def get_user_role(access_token,client_id):
    url="http://localhost:8080/admin/realms/flask-app/users/{0}/role-mappings/realm".format(client_id)
    headers = {
...
def get_admin_token():
    ...
    url="http://localhost:8080/admin/realms/{0}/users".format(realm)
    headers = {
