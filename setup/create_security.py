...
class CreateSecurity:
    ...
    def delete_realm(self,realm):
        url = 'http://localhost:8080/admin/realms/{0}'.format(realm)
        headers = {
    ...
    def create_client(self, realm,client):
        url = 'http://localhost:8080/admin/realms/{0}/clients'.format(realm)
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + self.admin_tokens_data["access_token"]
        }
    ...
    def list_users(self):
        ...
        url = 'http://localhost:8080/admin/realms/flask-app/users'
        headers = {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer ' + self.admin_tokens_data["access_token"]
        }
